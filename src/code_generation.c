...
string code_generation_from_svg(SvgInst *svg, unsigned int indent) {
    ...
    switch (svg->kind) {
        ...
        case Polyline: {
            ...
            APPEND_ARR(html, "<polyline fill=\"none\" stroke=\"");
            APPEND_ARR(html, svg->color_stroke);
    ...
    */
    ...
    switch (dom->dom_el) {
        ...
        case SVG: {
            ...
            while (child != NULL) {
                string child_html = code_generation_from_svg(child->svg, indent + 1);
        ...
        case Bold: {
            ...
            if (dom->children != NULL) {
                string content = code_generation_from_dom(dom->children->dom,
                                                          indent + 1); // Identation not relevant here
